...
func (m *model) openSelected() {
	...
	if it.isDir {
		...
		// Сбрасываем leftOffset для предотвращения подпрыгивания
		m.leftOffset = 0
		m.clampLeftOffset()
		return
	}
	...
	if strings.HasSuffix(strings.ToLower(it.path), ".md") {
		out, err := m.renderer.Render(string(data))
		if err != nil {
...
func (m *model) saveCurrent() {
	...
	if strings.HasSuffix(strings.ToLower(m.current), ".md") {
		out, err := m.renderer.Render(m.ta.Value())
		if err != nil {
			m.vp.SetContent(m.ta.Value())
		} else {
			m.vp.SetContent(out)
		}
	} else {
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		mPtr.ta.SetWidth(rightW - 2)
		mPtr.ta.SetHeight(mPtr.height - 3)
		mPtr.vp.Width = rightW - 2
		mPtr.vp.Height = mPtr.height - 3
	...
	case tea.KeyMsg:
		...
		if key.Matches(msg, mPtr.keys.ToggleHidden) {
			...
			mPtr.files = loadFiles(mPtr.cwd, mPtr.showHidden)
			// Сбрасываем leftOffset при изменении списка файлов
			mPtr.leftOffset = 0
			// Проверяем, что курсор в допустимых пределах
		...
		if key.Matches(msg, mPtr.keys.Tab) {
			...
			if mPtr.mode == "edit" {
				if strings.HasSuffix(strings.ToLower(mPtr.current), ".md") {
					out, err := mPtr.renderer.Render(mPtr.ta.Value())
					if err != nil {
						mPtr.vp.SetContent(mPtr.ta.Value())
					} else {
...
func (m model) View() string {
	...
	innerH := m.height - 4
	leftStyle := lipgloss.NewStyle().Width(m.leftWidth).Height(innerH).MarginTop(1).Padding(0, 1).Border(lipgloss.RoundedBorder())
	rightW := m.width - m.leftWidth - 4
	if rightW < 20 {
		rightW = 20
	}
	rightStyle := lipgloss.NewStyle().Width(rightW).Height(innerH + 1).MarginTop(1).Padding(0, 1).Border(lipgloss.RoundedBorder())
